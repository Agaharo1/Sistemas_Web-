...
export function nuevaPuja(req, res) {
  ...
  const nuevaPuja = Pujas.crearPuja(id_p, id_u, id);
...
export function enviarPuja(req, res) {
  const { valor, id, id_p, id_u } = req.body;
...
export function verPujas(req, res) {
  const pujas = Pujas.verTodasPujas();  // Asegúrate de que esto devuelva la lista
  const params = {
...
export function verPuja(req, res){
  const { id } = req.params;
  const puja = Pujas.getPujaByIdStmt(id);
  const params = {
